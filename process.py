...
def print_download_summary(
    ...
    print(
        f"📊 Download summary: {len(downloaded_files)} successful, {cached_count} cached, {download_count} downloaded"
    )
    if download_errors:
...
def generate_atari_image_urls(start: int = 1, end: int = 185) -> list[str]:
    ...
    if start < 1 or end < start:
        raise ValueError(
            "Invalid page range. Start page must be 1 or greater, and end page must be at least start page."
        )
    return [f"{ATARI_BASE_IMG_URL}{page}.gif" for page in range(start, end + 1)]


def convert_to_png(
    src_path: Path, dest_dir: Optional[str] = "png_output", verbose: bool = False
) -> Path:
...
def download_images(
    urls: list[str],
    save_dir: Optional[str] = "downloads",
    ...
    print_download_summary(
        downloaded_files, cached_count, download_count, download_errors, convert_errors
    )
...
def get_png_paths_for_pages(
    page_numbers: list[int], verbose: bool = False
) -> dict[int, Path]:
    ...
    if verbose:
        print(
            f"🔍 VERBOSE: Checking PNG files for {len(page_numbers)} pages: {page_numbers}"
        )
    ...
    if verbose:
        print(
            f"📊 VERBOSE: Found {len(png_paths)} PNG files, missing {len(missing_files)} files"
        )
        if missing_files:
...
def upload_images_for_pages(
    page_numbers: list[int], client: Any, verbose: bool = False
) -> dict[int, Any]:
    ...
    print(
        f"📤 Uploading {available_pages} specific images (requested: {requested_pages}, missing: {missing_pages})"
    )
    ...
    for i, (page_num, png_path) in enumerate(png_paths.items(), 1):
        try:
            ...
            if verbose:
                print(
                    f"✅ VERBOSE: Successfully uploaded page {page_num} as {gemini_file.name}"
                )
        except Exception as e:
    ...
    print(
        f"✅ Upload complete: {success_count}/{available_pages} images uploaded successfully"
    )
    ...
    if verbose:
        print(
            f"🔍 VERBOSE: Final page-to-file mapping: {list(page_to_gemini_file.keys())}"
        )

    return page_to_gemini_file


def create_page_to_file_mapping(
    gemini_files: list[Any], start_page: int, verbose: bool = False
) -> dict[int, Any]:
    ...
    if verbose:
        print(
            f"🔍 VERBOSE: Creating page-to-file mapping for {len(gemini_files)} files starting from page {start_page}"
        )
    ...
    if verbose:
        print(
            f"🔍 VERBOSE: Created mapping for pages: {sorted(page_to_gemini_file.keys())}"
        )
...
def save_transcription_to_markdown(
    transcription: str,
    page_range: list[int],
    output_dir: Optional[str] = "transcriptions",
) -> Path:
...
def identify_basic_programs(
    files: list[Any], client: Any, verbose: bool = False
) -> str:
    ...
    print(
        f"Program Location Extraction: Identifying BASIC programs across {len(files)} images..."
    )
...
def extract_program_source(
    ...
    print(
        f"Program Source Extraction: Extracting source code for '{program_name}' (pages: {pages_str}, {len(filtered_files)} images)..."
    )
...
def extract_program_source_optimized(
    files_for_program: list[Any],
    program_name: str,
    page_numbers: list[int],
    client: Any,
    verbose: bool = False,
    ...
    print(
        f"Program Source Extraction: Extracting source code for '{program_name}' (pages: {pages_str}, {len(files_for_program)} images)..."
    )
...
def parse_program_list(response_text: str, verbose: bool = False) -> list[dict]:
    ...
    if verbose:
        print(
            f"🔍 VERBOSE: Parsing program list from response ({len(response_text)} chars)"
        )
    ...
    try:
        ...
        if verbose:
            ...
            if programs:
                ...
                print(
                    f"🔍 VERBOSE: Programs span {len(total_pages)} unique pages: {sorted(total_pages)}"
                )
        ...
        if verbose:
            print(
                f"🔍 VERBOSE: Failed JSON text (first 500 chars): {json_text[:500]}..."
            )
        print(f"Raw response text (first 200 chars): {response_text[:200]}...")
        return []


def save_program_list_to_json(
    programs: list[dict], output_dir: Optional[str] = "transcriptions"
) -> Path:
...
def save_program_to_file(
    program_name: str, source_code: str, output_dir: Optional[str] = "programs"
) -> Path:
...
Requirements:
  ...
  - Internet connection for downloading images and API calls
    ...
    parser.add_argument(
        "--end", type=int, default=10, help="End page number (default: 10)"
    )
...
def handle_download_images_only(
    start_page: int, end_page: int, download_pause: float, verbose: bool
) -> None:
...
def handle_locate_programs_only(
    gemini_files: list[Any], output_dir: str, verbose: bool
) -> None:
    ...
    program_list_response = identify_basic_programs(
        gemini_files, client, verbose=verbose
    )
...
def handle_extract_source_only(
    program_list_path: str, output_dir: str, verbose: bool
) -> None:
    ...
    print(
        f"📊 Programs need {len(unique_pages)} unique pages: {', '.join(map(str, unique_pages))}"
    )
    ...
    try:
        ...
        for i, program in enumerate(programs, 1):
            ...
            if verbose:
                print(
                    f"🔍 VERBOSE: Program '{program_name}' expects {len(page_numbers)} pages: {page_numbers}"
                )
            try:
                files_for_program = []
                missing_pages = []
                ...
                if verbose:
                    print(
                        f"🔍 VERBOSE: Found files for {len(files_for_program)}/{len(page_numbers)} pages"
                    )
                    if missing_pages:
                        print(
                            f"🔍 VERBOSE: Missing pages for '{program_name}': {missing_pages}"
                        )
                if not files_for_program:
                    skip_msg = (
                        f"No files found for '{program_name}' on pages {page_numbers}"
                    )
                    print(f"⚠️  {skip_msg}")
                ...
                if missing_pages:
                    print(
                        f"⚠️  Partial data: missing {len(missing_pages)} pages for '{program_name}': {missing_pages}"
                    )
                print(f"🤖 Calling Gemini AI to extract source code...")
                source_code = extract_program_source_optimized(
                    files_for_program,
                    program_name,
                    page_numbers,
                    client,
                ...
                if not source_code.strip():
                    ...
                    skipped_programs.append((program_name, skip_msg))
                    continue
                output_path = save_program_to_file(
                    program_name, source_code, output_dir
                )
...
def handle_default_workflow(
    ...
    try:
        ...
        programs = load_program_list_from_json(
            Path(output_dir) / "program_list.json", verbose=verbose
        )
        if not programs:
            print("❌ No programs found in the program list file.")
            return
        unique_pages = get_unique_pages_from_programs(programs)
        page_to_gemini_file = create_page_to_file_mapping(
            gemini_files, start_page, verbose=verbose
        )
        saved_files = []
        skipped_programs = []
        failed_programs = []
            ...
            try:
                ...
                if verbose:
                    ...
                    if missing_pages_for_program:
                        print(
                            f"🔍 VERBOSE: Missing pages for '{program_name}': {missing_pages_for_program}"
                        )
                if not files_for_program:
                    skip_msg = (
                        f"No files found for '{program_name}' on pages {page_numbers}"
                ...
                if missing_pages_for_program:
                    print(
                        f"⚠️  Partial data: missing {len(missing_pages_for_program)} pages for '{program_name}': {missing_pages_for_program}"
                    )
                print(f"🤖 Calling Gemini AI to extract source code...")
                source_code = extract_program_source_optimized(
                    files_for_program,
...
def main() -> None:
    ...
    if args.download_images_only:
        handle_download_images_only(
            start_page, end_page, args.download_pause, args.verbose
        )
        return
    ...
    if args.extract_source_only:
        ...
        return
    handle_default_workflow(
        start_page, end_page, args.download_pause, args.output_dir, args.verbose
    )
